...
/atom/movable/screen/staminas/Click(location,control,params)
	if(isliving(usr))
		...
		to_chat(L, examine_block(span_notice("You have <b>[L.getStaminaLoss()]</b> stamina loss.<br>\
		<br>Your stamina buffer is <b>[round((L.stamina_buffer / buffer_max) * 100, 0.1)]%</b> full.")))
