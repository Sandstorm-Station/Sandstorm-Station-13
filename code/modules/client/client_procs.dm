...
/client/New(TopicData)
	...
	if (isnum(cached_player_age) && cached_player_age == -1) //first connection
		if (nnpa >= 0)
			...
			if (CONFIG_GET(flag/irc_first_connection_alert))
				send2adminchat("New-user", "[key_name(src)] is connecting for the first time!")
	...
	if(account_age >= 0 && account_age < nnpa)
		...
		if (CONFIG_GET(flag/irc_first_connection_alert))
			send2adminchat("new_byond_user", "[key_name(src)] (IP: [address], ID: [computer_id]) is a new BYOND account [account_age] day[(account_age==1?"":"s")] old, created on [account_join_date].")
	...
	if (oldcid)
		...
		if (oldcid != computer_id && computer_id != lastcid) //IT CHANGED!!!
			...
			if (!cidcheck_failedckeys[ckey])
				...
				send2adminchat("CidRandomizer", "[key_name(src)] has been detected as using a cid randomizer. Connection rejected.")
			...
			if (cidcheck_failedckeys[ckey])
				...
				send2adminchat("CidRandomizer", "[key_name(src)] has been allowed to connect after showing they removed their cid randomizer.")
