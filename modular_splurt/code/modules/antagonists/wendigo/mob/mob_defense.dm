/mob/living/carbon/wendigo/handle_environment(datum/gas_mixture/environment)
	...
	if(bodytemperature > BODYTEMP_HEAT_DAMAGE_LIMIT && !HAS_TRAIT(src, TRAIT_RESISTHEAT))
		switch(bodytemperature)
			if(280 to 300)
				throw_alert("tempfeel", /atom/movable/screen/alert/hot, 1)
				apply_damage(HEAT_DAMAGE_LEVEL_1*physiology.heat_mod, BURN)
			if(300 to 350)
				throw_alert("tempfeel", /atom/movable/screen/alert/hot, 2)
				apply_damage(HEAT_DAMAGE_LEVEL_2*physiology.heat_mod, BURN)
			if(350 to INFINITY)
				throw_alert("tempfeel", /atom/movable/screen/alert/hot, 3)
				if(on_fire)
		...
		if(!istype(loc, /obj/machinery/atmospherics/components/unary/cryo_cell))
			switch(bodytemperature)
				if(200 to 260)
					throw_alert("tempfeel", /atom/movable/screen/alert/cold, 1)
					apply_damage(COLD_DAMAGE_LEVEL_1*physiology.cold_mod, BURN)
				if(0 to 120)
					throw_alert("tempfeel", /atom/movable/screen/alert/cold, 2)
					apply_damage(COLD_DAMAGE_LEVEL_2*physiology.cold_mod, BURN)
				if(-INFINITY to 0)
					throw_alert("tempfeel", /atom/movable/screen/alert/cold, 3)
