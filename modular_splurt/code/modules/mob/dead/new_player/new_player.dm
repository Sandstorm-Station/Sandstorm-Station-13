/mob/dead/new_player/Topic(href, list/href_list)
	...
	var/datum/discord_link_record/player_link = SSdiscord.find_discord_link_by_ckey(client?.ckey)

	if(href_list["ready"])
		var/tready = text2num(href_list["ready"])
		if(tready != PLAYER_NOT_READY && !(player_link && player_link?.discord_id && player_link?.valid))
