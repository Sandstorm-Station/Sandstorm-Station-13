...
/datum/crew_manifest/ui_status(mob/user, datum/ui_state/state)
	return (isnewplayer(user) || isobserver(user) || isAI(user) || ispAI(user) || issiliconoradminghost(user)) ? UI_INTERACTIVE : UI_CLOSE

/datum/crew_manifest/ui_interact(mob/user, datum/tgui/ui)
	ui = SStgui.try_update_ui(user, src, ui)
	if (!ui)
		ui = new(user, src, "SplurtCrewManifest")
		ui.open()
...
/datum/crew_manifest/ui_data(mob/user) //Open slots stuff currently deactivated, still gotta figure out how to make it work
	/*
	...
	*/
	...
	return list(
		"manifest" = GLOB.data_core.get_manifest_tg()
		//"positions" = positions
	)
