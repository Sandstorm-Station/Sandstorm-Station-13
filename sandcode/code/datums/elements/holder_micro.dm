	...
	if(cooldown < world.time)
		for(var/mob/living/carbon/human/M in contents)
			...
			if(user.a_intent == "harm") //TO:DO, rework all of these interactions to be a lot more in depth
				visible_message("<span class='danger'>[user] slams their fist down on [M]!</span>", runechat_popup = 1)
				playsound(loc, 'sound/weapons/punch1.ogg', 50, 1)
				M.adjustBruteLoss(5)
				return
			if(user.a_intent == "disarm")
				visible_message("<span class='danger'>[user] pins [M] down with a finger!</span>", runechat_popup = 1)
				playsound(loc, 'sound/effects/bodyfall1.ogg', 50, 1)
				M.adjustStaminaLoss(10)
				return
			if(user.a_intent == "grab")
				visible_message("<span class='danger'>[user] squeezes their fist around [M]!</span>", runechat_popup = 1)
