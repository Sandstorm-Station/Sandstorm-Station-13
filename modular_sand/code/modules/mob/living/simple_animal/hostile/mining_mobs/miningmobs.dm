...
/mob/living/simple_animal/hostile/asteroid/AltClick(mob/living/carbon/slayer)
	...
	if(glorykill)
		if(do_mob(slayer, src, 10) && (stat != DEAD))
			var/message
			if(!slayer.get_active_held_item() || (!istype(slayer.get_active_held_item(), /obj/item/kinetic_crusher) && !istype(slayer.get_active_held_item(), /obj/item/gun/energy/kinetic_accelerator)) && glorymessageshand.len)
				message = pick(glorymessageshand)
			else if(istype(slayer.get_active_held_item(), /obj/item/kinetic_crusher) && glorymessagescrusher.len)
				message = pick(glorymessagescrusher)
			else if(istype(slayer.get_active_held_item(), /obj/item/gun/energy/kinetic_accelerator) && glorymessagespka.len)
