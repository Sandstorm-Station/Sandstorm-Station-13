...
/datum/preferences/proc/toggle_language(lang)
	if(lang in language)
		...
		if(CONFIG_GET(number/max_languages) == 0)
			to_chat(user, "<span class='danger'>Additional languages are disabled!</span>")
		else
			to_chat(user, "<span class='danger'>You can only have up to [CONFIG_GET(number/max_languages)] additional languages!</span>")
	else
		language += lang
